...
Manage issues, PRs, and labels using local YAML files instead of remote CI platforms.
...
For Docker CI/CD features (optional but recommended for E2E):
...
```
.claude/ci-filebase/
├── issues/           # Individual issue YAML files
│   └── {id}.yaml
├── prs/              # Individual PR YAML files
│   └── {id}.yaml
...
2. Create `labels.yaml` with default workflow labels
3. Create `counter.yaml` with zeroed counters
4. Report success

#### For `issue create`:

1. Increment issue counter
2. Create issue YAML file with:
   - ID, title, body
   - State: `open`
   - Labels from `--labels` flag
   - Timestamps
   - **REQUIRED:** DOD list with verification criteria
3. Return created issue ID

**DOD Requirement:** Every issue body MUST contain a "Definition of Done (DOD)" section with:

- Checklist items (`- [ ] item`)
- Verification method for each item (`- **Verification:** how to check`)

#### For `issue get`:

1. Read issue YAML file
...
| `type:decomposition`   | Spec/idea to decompose into tasks |
...
Claude: Initialized ci-filebase at .claude/ci-filebase/
  ...
  - Created labels.yaml with 15 default labels
  - Created counter.yaml

User: /ci-filebase issue create "Add user authentication" --body "Implement login/logout flow
...
- [ ] Logout endpoint invalidates session
  - **Verification:** POST /api/logout invalidates token, subsequent requests return 401
- [ ] Unit tests pass
  - **Verification:** Run npm test, all auth tests pass" --labels type:feature,state:backlog
...
### Example 9: E2E phase with Docker
