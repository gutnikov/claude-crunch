...
> **Note:** Required for E2E phase with Filebase. Enables local CI pipeline simulation and Docker-based staging.
...
> **Note:** Required for `/patrol` skill and E2E continuous monitoring. Can be skipped if observability is not needed, but `/patrol --logs` and `/patrol --metrics` will be unavailable.
...
### E2E Testability Infrastructure

> **Note:** Required for autonomous E2E validation. See `templates/5-e2e-testability.md`.

- [ ] Health endpoint implemented (`/health`)
- [ ] Health endpoint returns JSON with `status` and `checks` fields
- [ ] Health endpoint checks all critical dependencies (database, cache, external APIs)
- [ ] Metrics endpoint available (`/metrics`) for Prometheus scraping
...
curl -s http://localhost:8080/health | jq '.status, .checks'
# Expected: "healthy" and dependency status map
...
3. **Manual validation** - Skip continuous monitoring in E2E phase
...
4. **E2E support** - Full validation phase with local Docker staging
